...
Format_t errorColor = Format_t(Format_t::Color(Format_t::Color::rgb, {255, 0, 0})),
		 ...
		 userMenuColor =
			 Format_t(Format_t::Color(Format_t::Color::rgb, {128, 255, 0}),
...
{
	ostringstream s;
	s << animeName << " - s" << std::setw(2) << setfill('0') << seasonNumber << "e"
	  << setw(2) << setfill('0') << episodeNumber;
	return s.str();
}
...
std::string File::NewName()
{
	ostringstream s;
	s << animeName << " - s" << std::setw(2) << setfill('0') << seasonNumber << "e"
	  << setw(2) << setfill('0') << episodeNumber << GetFileExtension();
	return s.str();
}

std::string File::NewNameWithPath()
{
	return fm::GetExecutablePath() + NewName();
}
