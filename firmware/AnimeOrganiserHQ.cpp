...
//#define LOG_VERSION
// Jesli chcesz przesunac numery wszystkich odcinkow to pojdz na sam szczyt File.h i tam
// masz define ktory to obsluguje
...
int main(int argc, char *argv[])
{
	...
	else  /// Jesli musimy te dane sami pobrac
	{
		...
		// cout << errorColor << "Number of passed arguments is invalid! [" << dataColor
		// << argc
		//      << errorColor << "]" << endl
	...
	back2:
		...
		try {
			...
			cout << errorColor
				 << "Error! Please enter the CORRECT offset for subtitles: ";
			goto back2;
		}

		// Bierzemy z GetExecutablePath sciezke oraz GetSeasonNumber razem z wylapywaniem
		// bledow pobiera numer sezonu
	...
	for(auto dir: Subtitle::folders) {
		fm::ReadDirectory(File::GetDirectory() + dir + "/", subtitles,
						  Subtitle::Extensions(), false, true);
	}

	fm::ReadDirectory(File::GetDirectory(), subtitles, Subtitle::Extensions(), false,
					  true);
	...
	// Teraz trzeba je polaczyc w dwuwyymiarowej tablicy ktora ma pierwszy rozmiar
	// maxEpisodeNumber
	// + 1 I potem przelecenie wszyystkiego zmieniajac nazwe albo robiac okienka wyboru
	// (no ogolnie sporo przypadkow)
	...
	for(unsigned int i = 0; i < episodesSorted.size() - 1; i++) {
		   ...
		   episodesSorted[i].size() == 1)  // DONE
		{
			...
			cout << "\t" << SIColor << episodesSorted[i][0].NewName() << " - "
				 ...
				 << errorColor;
			cout << "\t" << SIColor << subtitlesSorted[i][0].NewName() << " - "
				 << successColor << "DONE!" << endl
				 << errorColor;
#endif
			// Zmiana nazwy odcinka anime i napisow plus przeniesienie napisow do
			// lokalizacji odcinka
		}
		else if(episodesSorted[i].size() == subtitlesSorted[i].size() &&
				subtitlesSorted[i].size() == 0)	 // DONE
		{
			// cout << "Nic nie robie bo nie ma ani napisow ani odcinka [" << i << "]" <<
			// endl;
		...
		else if(subtitlesSorted[i].size() > 1 && episodesSorted[i].size() > 1)	// DONE
		{
			int choosen1 = -1, choosen2 = -1;
			{
				...
				cout << "OH NOO! IMPOSSIBLE! (⊙_☉)\nI haven't been able to resolve "
						"occured problem by myself (˃̣̣̥⌓˂̣̣̥⋆)\n"
...
#ifdef LOG_VERSION
			fm::MoveFileLog(
				subtitlesSorted[i][choosen].OriginalNameWithPath(),
			...
			fm::MoveFile(subtitlesSorted[i][choosen].OriginalNameWithPath(),
						 ep.OriginalNameWithPath().replace(
							 ...
							 subtitlesSorted[i][choosen].OriginalName().substr(
								 subtitlesSorted[i][choosen].OriginalName().size() - 4)));
#endif
			remove(begin(subtitlesSorted[i]), end(subtitlesSorted[i]),
				   subtitlesSorted[i][choosen]);
