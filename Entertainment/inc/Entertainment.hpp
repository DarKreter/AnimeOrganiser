 ...
 * Created object has overloaded operator<< with ostream, so usage is just 'printing'
 * format object Foreground/background 4bit/24bit(8bit RGB) colors supported with
 * "Format_t::Color" Formatting like underline, blinking, italic font are also supported
 * with "Format_t::Mode"
 */
class Format_t {
	// Constants
	constexpr static char const *begin_sequence =
		"\033[";  // Start sequence of every escape seq
...
public:
	...
	// commented values are sequences, but mode supported combining all option in one
	// variable so each value is stored on separated bits
	enum Mode : uint8_t {
		normal		 = 0,
		italic		 = 1,	// 3;
		underline	 = 2,	// 4;
