...
#define DEFAULT_UP_KEYBOARD_KEYS                        {72,87,119}     //W w ↑
#define DEFAULT_DOWN_KEYBOARD_KEYS                      {80,83,115}     //S s ↓
#define DEFAULT_IN_KEYBOARD_KEYS                        {13}            //ENTER
#define DEFAULT_BACK_KEYBOARD_KEYS                      {27, 8}         //ESC, BACKSPACE
...
namespace menu
{
    ...
    #define DEFAULT_MENU_COLOR_SET                          {menu::DEFAULT_MENU_LOGO_FORE,menu::DEFAULT_MENU_LOGO_BACK,\
    ...
    class Menu_t
    {
        ...
        CreateMenu:
            (size_t -> okresla ile lini odstepu ma byc pomiedzy kolejnymi opcjami w menu,
             ...
             std::initializer_list<std::pair<string,WskaznikNaFunkcjeMenu>> -> {string, wskaznikNaFunkcje lub Funktor},
             sa to kolejne opcje menu: tekst i funkcja do wywolania podczas potwierdzenia wykonania tej opcji)
             Funkcja ma nic nie zwracac i nie przyjmowac argumentow

            std::initializer_list<keyboardKey> -> {kolejneChary} oznacza znaki dla ktorych ma byc wybierana wyzsza opcja w MENU
            Wybranie opcji jeden wyzszej

            std::initializer_list<keyboardKey> -> {kolejneChary} oznacza znaki dla ktorych ma byc wybierana nizsza opcja w MENU
            Wybranie opcji jeden nizszej

            std::initializer_list<keyboardKey> -> {kolejneChary} oznacza znaki dla ktorych ma byc wybierana opcja w MENU i wykonywane dzialania tej opcji
            Zatwierdzenie opcji

            std::initializer_list<keyboardKey> -> {kolejneChary} oznacza znaki dla ktorych ma byc cofniete MENU (powrot do poprzedniego)
            WOLNO TYLKO JEST TO MENU JEST NESTED

            std::vector<Color_e> -> kolory poukladane w kolejnosci oznaczaja jakiego koloru ma byc logo, jakiego koloru ma byc napis gdy opcja jest wybrana
            lub gdy nie jest wybrana | zobacz linia 45 Menu.h
        ...
        JESLI AKCJA POD WYBRANA OPCJA MENU MA POZOSTAC W MENU NIE NALEZY NA KONIEC TEJ FUNKCJI DODAWAC NIC

        JESLI AKCJA POD WYBRANA OPCJA MENU MA WROCIC DO MENU KTORE BYLO WCZESNIEJ NALEZY WYWOLAC .GoUpper()
        ...
        JESLI TA AKCJA MA ZMIENIC TEXT OBECNEJ OPCJI NALEZY UZYC FUNKCJI SKLADOWEJ ->Text(string) DLA ODPOWIEDNIEJ OPCJI,
        DO KTOREJ DOSTAJEMY SIE POPRZEZ []

        ZEBY ZMIENIC AKCJE DANEGO ELEMENU UZYWAMY .ChangeAction(WskaznikNaFunkcjeMenu); GDZIE WYSYLAMY AKCJE DO WYKONANIA
        ...
        */
        ...
        Color_e optionChoosenFore, optionNotChoosenFore, optionChoosenBack, optionNotChoosenBack, logoFore, logoBack;
        ...
        struct SpecialAction_t
        {
            enum Action_e
            {
                refreshAll, goToPreviousMenu, destroy
            };
        ...
        Menu_t(size_t, uint_least8_t,uint_least8_t,std::string, std::initializer_list<std::pair<std::string,WskaznikNaFunkcjeMenu>>,
       std::initializer_list<keyboardKey>, std::initializer_list<keyboardKey>,
       std::initializer_list<keyboardKey>, std::initializer_list<keyboardKey>,
       std::vector<Color_e>);

Menu_t(size_t, uint_least8_t,uint_least8_t,std::string, std::vector<std::pair<std::string,WskaznikNaFunkcjeMenu>>,
       std::initializer_list<keyboardKey>, std::initializer_list<keyboardKey>,
       std::initializer_list<keyboardKey>, std::initializer_list<keyboardKey>,
       std::vector<Color_e>);
    ...
    public:
        ...
        static Menu_t* CreateMenu(size_t, uint_least8_t, uint_least8_t, std::string, std::initializer_list<std::pair<std::string, WskaznikNaFunkcjeMenu>>,
            std::initializer_list<keyboardKey>, std::initializer_list<keyboardKey>,
            std::initializer_list<keyboardKey>, std::initializer_list<keyboardKey>,
            std::vector<Color_e>);

        static Menu_t* CreateMenu(size_t, uint_least8_t, uint_least8_t, std::string, std::vector<std::pair<std::string, WskaznikNaFunkcjeMenu>>,
            std::initializer_list<keyboardKey>, std::initializer_list<keyboardKey>,
            std::initializer_list<keyboardKey>, std::initializer_list<keyboardKey>,
            std::vector<Color_e>);
