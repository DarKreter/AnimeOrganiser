...
//#define LOG_VERSION
//Jesli chcesz przesunac numery wszystkich odcinkow to pojdz na sam szczyt File.h i tam masz define ktory to obsluguje
...
int main(int argc, char* argv[])
{
	...
	else ///Jesli musimy te dane sami pobrac
	{
		...
		ChangeColor(SIColor);    cout << "Enter the offset for subtitles: ";
		back2:
		ChangeColor(userColor); getline(cin, temp);
		try
	...
	for(auto dir: Subtitle::folders)
		fm::ReadDirectoryWS(File::GetDirectory() + sm::STWS(dir) + L"\\", subtitles, Subtitle::Extensions(), false, true);
	...
	if (subtitles.empty())
	{
		ChangeColor(errorColor);  cout << "No subtitles!" << endl;
		system("pause");
		exit(0);
	}
	...
	for (unsigned int i = 0; i < episodesSorted.size() - 1; i++)
	{
		...
		if (episodesSorted[i].size() == subtitlesSorted[i].size() && episodesSorted[i].size() == 1)			//DONE
		{
...
#ifdef LOG_VERSION
			fm::MoveFileWLog(subtitlesSorted[i][0].OriginalNameWithPath(), subtitlesSorted[i][0].NewName(), log);
			fm::RenameFileWLog(episodesSorted[i][0].OriginalNameWithPath(), episodesSorted[i][0].NewName(), log);
#else
			fm::MoveFileW(subtitlesSorted[i][0].OriginalNameWithPath(), subtitlesSorted[i][0].NewName());
		...
		else if (episodesSorted[i].size() == subtitlesSorted[i].size() && subtitlesSorted[i].size() == 0)	//DONE
		{
		...
		{
			cout << "Error! Couldn't find episode number " << i << " even though there are subtitles!" << endl;
			
			//Przekopiowanie wszystkich napisow jakie byly bez pary do bazy napisow z ostatniego odcinka
			move(begin(subtitlesSorted[i]), end(subtitlesSorted[i]),
				inserter(subtitlesSorted[subtitlesSorted.size() - 1], end(subtitlesSorted[subtitlesSorted.size() - 1])));
		...
		{
			cout << "Error! Couldn't find subtitle number " << i << " even though there is episode!" << endl;

			//Przekopiowanie wszystkich napisow jakie byly bez pary do bazy napisow z ostatniego odcinka
			move(begin(episodesSorted[i]), end(episodesSorted[i]),
				inserter(episodesSorted[episodesSorted.size() - 1], end(episodesSorted[episodesSorted.size() - 1])));
		...
		else if (subtitlesSorted[i].size() > 1 && episodesSorted[i].size() > 1) //DONE
		{
			int choosen1 = -1, choosen2 = -1;
			{
				...
				(2, 1, 55, "Found too much episodes and subtitles with number " + to_string(i) + "!\nFirstly please choose which episode is correct!",
					x,
			...
			{
#ifdef LOG_VERSION
				fm::MoveFileWLog(subtitlesSorted[i][choosen2].OriginalNameWithPath(), subtitlesSorted[i][choosen2].NewName(), log);
				fm::RenameFileWLog(episodesSorted[i][choosen1].OriginalNameWithPath(), episodesSorted[i][choosen1].NewName(), log);
#else
				fm::MoveFileW(subtitlesSorted[i][choosen2].OriginalNameWithPath(), subtitlesSorted[i][choosen2].NewName());
				fm::RenameFileW(episodesSorted[i][choosen1].OriginalNameWithPath(), episodesSorted[i][choosen1].NewName());
#endif

				auto newEnd1 = remove(begin(subtitlesSorted[i]), end(subtitlesSorted[i]), subtitlesSorted[i][choosen2]);
				subtitlesSorted[i].erase(newEnd1, end(subtitlesSorted[i]));

				auto newEnd2 = remove(begin(episodesSorted[i]), end(episodesSorted[i]), episodesSorted[i][choosen1]);
		...
		else if (episodesSorted[i].size() > 1) //DONE
		{
			...
			if (choosen != -1) //Jesli cos zostalo wybrane
			{
				...
				auto newEnd = remove(begin(episodesSorted[i]), end(episodesSorted[i]), episodesSorted[i][choosen]);
		...
		else if (subtitlesSorted[i].size() > 1) //DONE
		{
			...
			meni = menu::Menu_t::CreateMenu
			(2, 1, 55,	"Found too much subtitles for episode number " + to_string(i) + "!\nPlease choose which one is correct!",
				x,
				DEFAULT_UP_KEYBOARD_KEYS, DEFAULT_DOWN_KEYBOARD_KEYS, DEFAULT_IN_KEYBOARD_KEYS, {},
				DEFAULT_MENU_COLOR_SET);
			...
			{
#ifdef LOG_VERSION
				fm::MoveFileWLog(subtitlesSorted[i][choosen].OriginalNameWithPath(), subtitlesSorted[i][choosen].NewName(), log);
				fm::RenameFileWLog(episodesSorted[i][0].OriginalNameWithPath(), episodesSorted[i][0].NewName(), log);
#else
				fm::MoveFileW(subtitlesSorted[i][choosen].OriginalNameWithPath(), subtitlesSorted[i][choosen].NewName());
				fm::RenameFileW(episodesSorted[i][0].OriginalNameWithPath(), episodesSorted[i][0].NewName());
#endif


				auto newEnd = remove(begin(subtitlesSorted[i]), end(subtitlesSorted[i]), subtitlesSorted[i][choosen]);
	...
	for (auto& ep : episodesSorted[episodesSorted.size() - 1])
	{
		...
		x.push_back({ "I've seen Enough. I'm Satisfied.", [&meni]()
			{	menu::ClearScreen(); ent::ChangeColor(SIColor); cout << "SUCCESS!" << endl; _getch(); exit(0); } });

		meni = menu::Menu_t::CreateMenu
		(2, 1, 55, "Choose subtitle file for this episode:\n" + sm::WSTS(ep.OriginalName()),
			x,
			DEFAULT_UP_KEYBOARD_KEYS, DEFAULT_DOWN_KEYBOARD_KEYS, DEFAULT_IN_KEYBOARD_KEYS, {},
			DEFAULT_MENU_COLOR_SET);
		...
		if (choosen != -1) //Jesli cos zostalo wybrane
		{
			...
			fm::MoveFileWLog(subtitlesSorted[i][choosen].OriginalNameWithPath(),
				ep.OriginalNameWithPath().replace(ep.OriginalNameWithPath().size() - 4, 4,
					subtitlesSorted[i][choosen].OriginalName().substr(subtitlesSorted[i][choosen].OriginalName().size() - 4)), log);
#else
			fm::MoveFileW(subtitlesSorted[i][choosen].OriginalNameWithPath(),
				ep.OriginalNameWithPath().replace(ep.OriginalNameWithPath().size() - 4, 4,
					subtitlesSorted[i][choosen].OriginalName().substr(subtitlesSorted[i][choosen].OriginalName().size() - 4)));
#endif
